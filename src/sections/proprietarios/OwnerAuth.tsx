    ...
    try {
      ...
      if (!res.ok) {
        toast.error('Não foi possível entrar', {
          description: data?.message || 'E-mail ou senha inválidos.',
        });
        return;
      }
      ...
      if (data?.owner?.must_change_password) {
        toast.success(`Bem-vindo, ${data?.owner?.nome || 'proprietário'}!`, {
          description: 'Sua senha provisória precisa ser alterada no primeiro acesso.',
          duration: 6000,
        });
      } else {
        toast.success(`Bem-vindo, ${data?.owner?.nome || 'proprietário'}!`, {
            ...
            <button
              ...
              onClick={() =>
                toast.info('Recuperação de senha', {
                  description:
                    'Em breve você poderá redefinir a senha pelo portal do proprietário.',
                })
              }
              className="cursor-pointer text-zinc-600 underline dark:text-zinc-400"
            >
              Esqueci a senha
            </button>
